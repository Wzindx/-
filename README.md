...
README 面向项目整体功能与主分支能力进行说明。具体版本号、变更记录、APK 资产和发布时间请以 GitHub Releases 页面为准。下载入口使用 `releases/latest` 稳定链接，会自动指向当前最新版本。
...
6. 删除 ImageForge 历史记录时，会同步删除 App 私有目录中的对应图片副本；但不会删除已经手动导出到系统相册或自定义目录的图片。

这意味着 ImageForge 支持“不下载到相册也能分享”，也能在删除记录时释放 App 私有存储空间。
...
- 选择参考图后立即读取并缓存图片数据，真正生成时直接复用缓存，降低系统相册 Uri 失效导致的生成失败概率。
...
请从 GitHub Releases 下载最新 APK。该链接是稳定最新版入口，会自动跳转到当前最新版本：
...
- 使用仓库提交的 Gradle Wrapper 构建，避免本机 Gradle 版本差异
...
当前主分支的构建链版本以仓库中的 Gradle Wrapper、`gradle/libs.versions.toml` 和 GitHub Actions 配置为准。

请不要单独升级 Android Gradle Plugin 到新的大版本。构建工具链大版本升级应在单独分支中同时验证 Gradle Wrapper、Android Gradle Plugin、Kotlin、lint、Debug APK、Release APK 和 Release workflow。
...
### 删除 ImageForge 里的历史记录后，图片文件还在吗？

删除记录时，ImageForge 会同步删除 App 私有目录 `files/generated_images` 中对应的图片副本，用于释放应用私有存储空间。

如果你已经在详情页点击“保存”，把图片导出到系统相册或自定义保存目录，那么这些外部文件不会被删除。
